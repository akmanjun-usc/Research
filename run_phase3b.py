...
from fitness import fitness_n2
...
def main() -> None:
    if not CHECKPOINT.exists():
        raise FileNotFoundError(
            ...
            "Run: .venv/bin/python neural_bm.py --train --seed 32"
        )
    ...
    model.eval()
    print(f"  N2 loaded (seed={ckpt.get('seed', '?')}, "
    ...
    for rng_seed in range(N_SEEDS):
        ...
        def generation_callback(snap: dict) -> None:
            ...
            if snap.get("is_complete"):
                print(_GEN_SEP, flush=True)
                print(
                    f"  stopped: plateau={snap['plateau']} >= {plateau_patience}",
                    flush=True,
                )
