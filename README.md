...
A simulation study of convolutional/trellis codes operating over an AWGN channel
**plus periodic sinusoidal interference**. The central question:

> Can co-designing the trellis *and* a constrained learned decoder for a realistic
> impairment family beat optimizing either one independently?

The goal is **robustness and graceful degradation under sinusoidal interference**,
*not* beating polar codes on clean AWGN. The headline result is honest and partly
negative: a learned branch-metric decoder (N2) clearly beats the classical
mismatched decoder, but an evolutionary search over trellis structures converges
back to the standard NASA K=7 code — no better trellis was found.
